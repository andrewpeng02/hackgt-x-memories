...
type TreeProps = {name: string, stage: number, treeName: string, uid: string, top: number, left: number}
...
function Tree({name, stage, treeName, uid, left, top}: TreeProps) {
  const [firTreeAssets] = useAssets([require('../assets/fir_tree/1.png'), require('../assets/fir_tree/2.png'), require('../assets/fir_tree/3.png'), require('../assets/fir_tree/4.png'), require('../assets/fir_tree/5.png')])
  const [middleLaneTreeAssets] = useAssets([require('../assets/middle_lane_tree/1.png'), require('../assets/middle_lane_tree/2.png'), require('../assets/middle_lane_tree/3.png'), require('../assets/middle_lane_tree/4.png'), require('../assets/middle_lane_tree/5.png')])
  ...
  return (
    <View style={[treeStyles.container, { left: left, top: top }]}>
      <Button buttonStyle={{backgroundColor: null, flexDirection: 'column', flex: 1}} onPress={handlePress}>
        <Image source={{uri: asset.uri}} style={[treeStyles.image, dimensions]} />
...
function getTrees(treeInfos: [TreeInfo]) {
  ...
  return treeInfos.map(treeInfo => {
    ...
    return (
      <Tree {...treeInfo} left={leftRandomized} top={topRandomized} key={treeInfo.uid} />
    )
  })
}
...
export default function HomeScreen() {
  ...
  useEffect(() => {
    setTreeInfos([
      {name: 'my namea sdf asd', stage: 2, treeName: 'fir_tree', uid:'asd9fjas9d-d'},
      ...
      {name: 'my name', stage: 0, treeName: 'middle_lane_tree', uid:'asd9rfjas9-d'},
    ])
  }, [])
